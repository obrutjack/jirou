...
def test_diagnostics_run_gate_once_and_preserve_verdict(monkeypatch, capsys, returncode):
    ...
    monkeypatch.setenv("BLACK_NUM_WORKERS", str(_step()["env"]["BLACK_NUM_WORKERS"]))
    phases = []
...
def test_gate_preserves_scope_workers_and_baseline_on_black_result(
    monkeypatch, tmp_path, black_exit
):
...
def test_floor_runs_the_exact_ci_black_command_and_worker_budget():
    ...
    step = _step()
    command = f"BLACK_NUM_WORKERS={step['env']['BLACK_NUM_WORKERS']} {step['run']}"
...
def test_two_worker_real_gate_preserves_ratchet(monkeypatch, tmp_path, known, new, expected):
