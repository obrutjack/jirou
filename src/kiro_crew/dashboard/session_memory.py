...
class SessionMemorySampler:
    ...
    def _lineage(self, rows: list[dict[str, object]]) -> "tuple[dict[str, TreeNode], int]":
        """Who opened whom, folded from the crew logs, and how many session logs
        the scan left unread past ``TREE_UNIT_CAP``; ``({}, 0)`` with the crew
        log off. The live rows' logs are admitted first (their ACP session ids
        name their units), so past the cap it is closed sessions' logs that go
        unread, not a row's on screen -- while the live rows fit in the cap.

        BLOCKING (a ``stat`` per live row, a directory listing and a ``stat`` per
        session log), so it is called from :meth:`_blocking_sample`, never from
        the coroutine.
        ...
        return nodes, self._tree.omitted
    ...
    async def sample(
        ...
        lineage, lineage_omitted = lineage_sample
        assert isinstance(lineage, dict)
        assert isinstance(lineage_omitted, int)
        now_wall = time.time()
        ...
        return {
            ...
            "totals": {
                ...
                # Session logs the lineage scan left unread past TREE_UNIT_CAP.
                # Live rows' logs are admitted first, so these are closed
                # sessions' logs and no row on screen is affected while the
                # live rows fit in the cap; said on every payload because a
                # store that large is worth knowing about.
                # 0 within the cap or with the crew log off.
