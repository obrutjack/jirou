...
_STAGE_SUBAGENT_POLL_ROUND_CAP = 450
...
def _cancel_release_owns_handoff(
    ...
    *,
    ...
    """Whether the release seam, rather than a stage controller, owns handoff."""
...
async def _release_cancelled_plan_boundary(
    ...
    *,
    ...
    """Settle one cancelled plan, publish its stop, then release admission."""
    ...
    async with slot._lock:
        ...
        owed = slot.take_pending_subagent_deliveries(removed_contents)
        if owed and state.subagents is not None:
            try:
                settlement = state.subagents.settle_queued_delivery(owed)
                if asyncio.iscoroutine(settlement):
                    await settlement
            except Exception:
                logger.warning(
                    "Could not settle cancelled stage deliveries for slot %s",
                    slot.key,
                    exc_info=True,
                )
        if state.subagents is not None:
            ...
            discard_failures = getattr(state.subagents, "discard_report_failures", None)
...
def _queue_consumed_stage_resume(
    ...
    *,
    ...
    """Queue the continuation owed by a consumed interrupted stage."""
    if (
...
async def _cancel_stage_subagents(state: "DashboardState", slot: "_ChatSlot") -> None:
    """Cancel stage children under every captured parent before boundary release."""
    manager = state.subagents
    if manager is None:
        return
    parent_keys = set(stage_boundary_for(slot).parent_session_keys)
    parent_keys.add(f"dashboard:{slot.key}")
    for parent_key in sorted(parent_keys):
        cancellation = manager.cancel_for_parent(parent_key)
...
async def _settle_stage_delivery(
    ...
    *,
    ...
    max_rounds = (
        min(
            tracker.stage_timeout_seconds // 4,
            _STAGE_SUBAGENT_POLL_ROUND_CAP,
        )
        if tracker.stage_timeout_seconds
        else _STAGE_SUBAGENT_POLL_ROUND_CAP
    )
    rounds = 0
    clean_passes = 0
...
async def api_chat_plan_action(request: web.Request) -> web.Response:
    ...
    if action == "cancel":
        ...
        if not slot._in_stage_execution:
            ...
            # The plan-action route joins the controller before the shared release
            # seam publishes the terminal frame and starts one queued turn.
            from kiro_crew.dashboard.chat_handlers import _cancel_stage_controller
