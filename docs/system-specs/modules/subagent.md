...
Every run record rebuilt from admitted parameters or an existing run retains its
stage-boundary owner, including queued starts and stops, continuations, recovery
respawns, automatic follow-ups, and synthetic failures. A retry admitted while
its parent has an active boundary captures that current owner; only a retry with
no active parent boundary falls back to the failed run's owner. Channel parents
resolve that owner through their effective session identity even without an open
dashboard surface.
...
- **Failed report settlement is boundary-owned and bounded.** Only a report with a non-empty `_stage_boundary_owner` enters the failure latch. Entries are keyed by `(parent_session_key, boundary_owner)`, retain at most 500 scopes, and saturate each exact count at 65 (64 failures plus a count-overflow sentinel scoped to that parent and owner). They are observed rather than consumed: each failed completion keeps Go blocked until its report is successfully redelivered. If the 500-scope capacity is exhausted, evicted and newly dropped failures move into one bounded global overflow entry that every owned boundary consults. Scope identity is unavailable in that state, so all barriers fail closed until a retained payload reaches eviction: failed redelivery keeps its `SubagentInfo` and moves that payload's contribution from the global sentinel to the exact parent-owner key; successful redelivery settles the debt before record removal. Explicit finished-run deletion follows the same rule while the boundary is active and returns `completion_delivery_pending` if redelivery still fails. Cancelling or discarding the boundary clears only its matching record contributions, after which the payload may be removed without pausing another plan. A hard stop discards the owning stage boundary, while an ordinary non-Autopilot report is unowned, so neither can halt or advance a later stage.
