...
| Id | Guarantees | Pinned by | Constrains |
|---|---|---|---|
| H9 | `kiro-cli` remains the default branch of spawn-argv resolution, keeping its pre-spawn agent materialization (`kiro-cli` discovers selectable modes from `~/.kiro/agents/*.json` at startup, so a later `set_mode` fails with "Mode not found" without it) and its `--model` pin (the only way to run a model outside the agent's provider). A refactor that treats Kiro as one entry among N drops both. The Kiro spawn lives in its own harness, so no other host shares the code that carries them. | `test_harness_parity.py::test_kiro_spawn_argv_keeps_its_own_branch`, `::test_codex_resolves_its_own_adapter_and_declares_its_own_handshake`, `test_acp_harness_contract.py::test_kiro_spawn_argv_names_the_agent_and_the_model` | `acp/harness/kiro.py` (`KiroHarness.resolve_spawn`) |
...
| H15 | Interactive stdin ownership never depends on a harness's native shell tool. No backend (kiro-cli `execute_bash`, the Claude harness Bash tool) hands Kiro Crew the child's stdin or a PTY, so a `waiting_input` shown for such a child is a REPORT, never a takeover: the pre-dispatch classifier (`liveness.classify_interactive_command`, [acp-client.md](acp-client.md) § Interactive-command policy) yields the lane slot and names the prompt, and `agent.interactive_command_policy` decides between a non-lethal cancel and a declared wait -- identically on every backend, because it keys on the trusted `AcpEvent.shell_command` rather than harness machinery. The one path where Kiro Crew would OWN the child (a `kirocrew-core` terminal tool holding stdin and a PTY) is a separable capability that ships in a follow-up PR; a harness that later exposes a stdin-capable shell tool is integrated by an adapter beside it, never by reaching into the harness process. The declared gap per backend is the "blocked on stdin" row of the RFC's platform-evidence table. | review-only (`AUTOSDE.yaml` → `harness-parity`), plus `test_interactive_command_detection.py` (classifier and policy are backend-independent) | `acp/liveness.py` (classifier + evidence table), `acp/session_handle.py` (`interactive_command_policy`) |
| H16 | A harness-native subtask (kiro-cli `use_subagent`, KAS `agentSubtaskId` / pipeline stage, the Claude adapter's in-harness Task tool) is COUNTED at its parent session and scheduled NOWHERE ELSE. The parent `AcpSessionHandle.native_child_sessions` is the only place a native child has a number; it is never charged a `HostBudget` slot (reported under `snapshot()["uncharged"]["native_children"]` via `report_native_children`), never given a lane slot or a taskq row, and the recovery boundary is the parent session — one `session/cancel` on the parent covers every child, a child-routed `kirocrew/status` is rejected (`child_origin`), and a `spawn_continue`-style resume of a child id is refused with `native_child_not_resumable` naming the parent. Per-backend identity: kiro-cli yes (child-routed frames under both `session/update` spellings + the roster it owns), KAS yes (`_meta.kiro.agentSubtaskId`), **Claude none** — its counter stays 0 not because there are no children but because the harness exposes none; this is a declared capability gap, and the liveness oracle still bounds the Claude parent turn exactly as it bounds a kiro-cli parent. A Kiro Crew `spawn_run` child is a task row with its own handle, so native grandchildren under it are counted on THAT handle only (the Kiro Crew row is the boundary). On a shared runtime with several registered sessions an unannounced child frame names no owner and is dropped (counted), so the number is "observed", never "scheduled". A harness that later exposes per-child cancel/resume is integrated by adding a row adapter, never by teaching the scheduler to pause one child. | `test_native_subagent_boundary.py` (every test; the §10 scenario map is in its module docstring), plus `test_acp_structured_status.py::test_child_routed_frame_status_is_rejected_and_not_reattributed` | `acp/session_handle.py` (`native_child_sessions`, `_note_native_child`, `native_child_resume_refusal`, `report_native_children`), `mcp_gateway/host_budget.py` (`report_uncharged`, `uncharged`), `acp/runtime.py` (child-frame routing) |
