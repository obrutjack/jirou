...
artifact names. `backend-lint` uses large plus `BLACK_NUM_WORKERS=2` on its
Black step on both fleet and hosted runners. `scripts/ci_black_diagnostics.py`
runs the formatting gate once, preserves its failure status and stderr, and
records bounded cgroup readings and child peak RSS. Worker count alone does not
bound retained formatting trees. `scripts/bounded_black.py` adapts the pinned
native Black CLI to at most two spawn workers, each retired after one file.
Native discovery, configuration, exclusions, caches and AST checks remain intact.
On Linux the launcher and workers have a 2 GiB per-process address-space ceiling;
macOS and Windows retain recycling without that Linux-only ceiling. Incomplete
...
prepare-pr profile keeps the same diagnostic command. Ratchet scope, graduates
and prune-only baseline refresh remain unchanged.
